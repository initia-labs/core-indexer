...
func (r *NftRepository) GetCollections(pagination dto.PaginationQuery, search string) ([]db.Collection, int64, error) {
	...
	if pagination.CountTotal {
		var err error
		safeSearch := regexp.QuoteMeta(search)
		total, err = db.CountWithTimeout(r.db.Model(&db.Collection{}).Where("name ~* ? OR id = ?", safeSearch, strings.ToLower(safeSearch)), r.countQueryTimeout)
		if err != nil {
...
func (r *NftRepository) GetCollectionMutateEvents(pagination dto.PaginationQuery, collectionAddress string) ([]dto.MutateEventModel, int64, error) {
	...
	if pagination.CountTotal {
		var err error
		total, err = db.CountWithTimeout(r.db.Model(&db.CollectionMutationEvent{}).Where("collection_mutation_events.collection_id = ?", collectionAddress), r.countQueryTimeout)
		if err != nil {
...
func (r *NftRepository) GetNftMutateEvents(pagination dto.PaginationQuery, nftAddress string) ([]dto.MutateEventModel, int64, error) {
	...
	if pagination.CountTotal {
		var err error
		total, err = db.CountWithTimeout(r.db.Model(&db.NftMutationEvent{}).Where("nft_mutation_events.nft_id = ?", nftAddress), r.countQueryTimeout)
		if err != nil {
...
func (r *NftRepository) GetNftTxs(pagination dto.PaginationQuery, nftAddress string) ([]dto.NftTxModel, int64, error) {
	...
	if pagination.CountTotal {
		var err error
		total, err = db.CountWithTimeout(r.db.Model(&db.NftTransaction{}).Where("nft_transactions.nft_id = ?", nftAddress), r.countQueryTimeout)
		if err != nil {
